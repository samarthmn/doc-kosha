-- Email-only comments: remove anonymous identity mode configuration

alter table public.links
  drop constraint if exists links_comments_identity_mode_check;

alter table public.links
  drop column if exists comments_identity_mode;

-- Store additional message metadata for email identity + deletions.
alter table public.comment_messages
  add column if not exists author_email text null,
  add column if not exists state text not null default 'active' check (state in ('active', 'deleted')),
  add column if not exists deleted_at timestamptz null;

-- Recreate resolve function without comments_identity_mode
