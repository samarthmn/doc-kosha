...
import { presignUploadPart, type LogicalBucket } from "@/server/storage";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (partNumber > intent.totalParts) {
      return NextResponse.json(
        { error: "Part number exceeds multipart upload intent" },
        { status: 400 },
      );
    }
    ...
    if (access.status === "unavailable") {
      console.error("[Storage Multipart SignPart] permission check failed", {
        workspaceId,
        error: access.error,
      });
      return NextResponse.json(
    ...
    const result = await presignUploadPart({
      logicalBucket: logicalBucket as LogicalBucket,
      path: storagePath,
      uploadId,
      partNumber,
    ...
    if (!result.ok) {
      console.error("[Storage Multipart SignPart] failed", {
    ...
    console.error("[Storage Multipart SignPart] Error:", err);
